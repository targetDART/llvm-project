...
[[nodiscard]] tdrc TD_Task_Queue::offer_task(td_task_t* task) {
    ...
    cost.fetch_add(task->cached_total_sizes, MEM_ORDER);
...
//        }
    ...
    size.fetch_add(-1, MEM_ORDER);
    cost.fetch_sub(entry->cached_total_sizes, MEM_ORDER);
    return entry;
}

td_task_t *TD_Task_Queue::getTask() {
    TRACE_START("get_task_from_base_queue\n");    
    td_task_t *task = poll_task(nullptr);
...
void TD_Task_Queue::addTask(td_task_t *task) {
    ...
    auto result = offer_task(task);
    if (result != TARGETDART_SUCCESS) {
        DP("Failed to add task (%ld%ld) to queue\n", task->uid.rank, task->uid.id);
        llvm::report_fatal_error("Failed to add task to queue. Queue is full or corrupted.");
    }
    TRACE_END("add_task_to_base_queue\n");
}

size_t TD_Task_Queue::getSize() {
    return size.load(MEM_ORDER);
}

COST_DATA_TYPE TD_Task_Queue::getCost() {
    return cost.load(MEM_ORDER);
}